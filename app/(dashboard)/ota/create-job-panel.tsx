...
export function CreateJobPanel({
  ...
  const [targetMode, setTargetMode] = useState<"all" | "filtered" | "manual">("all");
  ...
  const targetDevices = useMemo(() => {
    if (targetMode === "all") return productDevices;
    if (targetMode === "filtered") return productDevices.filter((d) => d.state === stateFilter);
    return productDevices.filter((d) => selectedDeviceIds.has(d.id));
  }, [productDevices, targetMode, stateFilter, selectedDeviceIds]);
  ...
  function selectAllProductDevices() {
    setSelectedDeviceIds(new Set(productDevices.map((d) => d.id)));
  }
      ...
      {targetMode === "manual" && (
        <div className="flex flex-col gap-2">
          {productDevices.length === 0 ? (
            <p className="text-xs text-muted-foreground">Select a product to list its devices.</p>
          ) : (
            <>
              <div className="flex items-center justify-between">
                <label className={LABEL_CLASS}>Devices ({productDevices.length})</label>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={selectAllProductDevices}
              ...
              <div className="rounded-md border border-border max-h-56 overflow-y-auto divide-y divide-border">
                {productDevices.map((d) => (
                  ...
                  >
                    ...
                    />
                    <span className="font-mono text-xs font-semibold text-foreground w-24 shrink-0">
                      {d.serial}
                    </span>
                    <span className="font-mono text-xs text-muted-foreground flex-1">{d.mac}</span>
