...
import { triggerOtaJob } from "./actions";
...
type DeviceOption = { id: string; serial: string; state: string; fwVersion: string | null; productId: string };
...
export function UpdatePanel({
  ...
  const [targetMode, setTargetMode] = useState<"outdated" | "all" | "filtered">("outdated");
  ...
  const productDevices = devices.filter((d) => d.productId === productId);

  let targetDevices = productDevices;
  if (targetMode === "filtered") {
    targetDevices = productDevices.filter((d) => d.state === stateFilter);
  } else if (targetMode === "outdated") {
    // devices not already running the selected firmware version
    targetDevices = selectedFirmware
      ? productDevices.filter((d) => d.fwVersion !== selectedFirmware.version)
      : [];
  }
  ...
  function handleTrigger() {
    ...
    startTransition(async () => {
      const result = await triggerOtaJob({
        firmwareId,
  ...
  if (!canTrigger) {
    return (
      <div className="rounded-lg border border-amber-200 bg-amber-50 text-amber-800 text-sm px-4 py-3 mb-6">
        Your account has viewer access — you can see firmware and OTA status below, but only an
        account admin can trigger an update.
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-4 mb-6 flex flex-col gap-4">
      <h2 className="text-sm font-semibold text-foreground">Push an Update</h2>
          ...
          <div className="flex items-center gap-3 h-8 flex-wrap">
            <label className="flex items-center gap-1.5 text-sm text-foreground">
              <input
                type="radio"
                name="targetMode"
                checked={targetMode === "outdated"}
                onChange={() => setTargetMode("outdated")}
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {isPending ? "Starting..." : "Start Update"}
        </button>
        {error && <p className="text-xs text-rose-600">{error}</p>}
        {justCreated && !error && <p className="text-xs text-emerald-600">Update started.</p>}
      </div>
    </div>
  );
}
